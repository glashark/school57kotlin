...
fun <A, R> logCalls(name: String, f: (A) -> R): (A) -> R {
    return { arg: A ->
        ...
        val ans = f(arg)
        println("[$name] вернула результат: $ans")
        ans
    }
}
...
fun <T> retry(times: Int, f: () -> T): () -> T {
    return retryFunction@{
        ...
        while (i > 0) {
            try {
                return@retryFunction f()
            } catch (e: Throwable) {
                exc = e
                i--
            }
        }
        throw exc ?: Exception("All attempts failed")
    }
}
...
fun <A, R> timed(name: String, f: (A) -> R): (A) -> R {
    return { arg: A ->
        ...
        val timee = ChronoUnit.MILLIS.between(start, end)
        println("[$name] выполнено за $timee мс")
        ans
    }
}
